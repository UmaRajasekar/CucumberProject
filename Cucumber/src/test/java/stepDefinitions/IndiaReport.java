...
public class IndiaReport {
    ...
    public void keyword_something_is_entered_in_search_bar(String strArg1) throws Throwable {
        System.out.println("Entering keyword in search bar:"+strArg1);
    }

    @Then("^opening browser should be validated$")
    public void opening_browser_should_be_validated() throws Throwable {
        System.out.println("Validating opened browser");
    }

    @Then("^Results should be available under below sections:$")
    public void results_should_be_available_under_below_sections(DataTable data) throws Throwable {
        System.out.println("Opening Results from the search bar and results should be available from below sections");
        List<List<String>> obj=data.raw();
        for (List<String> list : obj) {
			for (String string : list) {
				System.out.println("Section displayed:"+string);
			}
		}
    }
